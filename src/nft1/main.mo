...
shared (install) actor class erc721_token(init_minter: Principal) = this {
  ...
  var properties : [(Text, [(Text, Nat)])] = [
  ("Backgrounds", [("Blue", 15), ("Dfinity", 8), ("Dots", 15), ("Galaxy", 4), ("Ocean", 7), ("Plain", 16), ("Royal", 13),  ("Skull", 12),  ("Tv", 10)]),
  ("Pots", [("Clay", 14), ("Fancy", 11), ("ICP", 6), ("Infernal", 9), ("Jungle", 10), ("Marble", 11), ("Obsidian", 9),  ("PC", 8),  ("Sludge", 9), ("Water", 13)]),
  ("Stems", [("Board", 8), ("Cables", 8), ("Candy", 7), ("Dfinity", 4), ("Dried", 10), ("Fancy", 6), ("Galaxy", 5),  ("Goldfish", 9),  ("Green", 11), ("Muscles", 7), ("Plumbing", 9), ("Suit", 7), ("Thorns", 9)]),
  ("Petals", [("Candy", 9), ("Chrysanthemum", 8), ("Devil", 9), ("Dfinity", 6), ("Ghost", 10), ("Muscles", 12), ("Purple", 14),  ("Red", 17),  ("Yellow", 15)]),
  ("Globalacc", [("Band", 28), ("Butterfly", 25), ("Hearts", 27), ("Motoko", 20)]),
  ("Special Pots", [("Fancy2", 33), ("Japanese", 32), ("Protein", 35)]),
  ("Accesory", [("Crown", 7), ("Facemask", 12), ("GreenSunglass", 15), ("Hat", 14), ("Headphone", 13), ("Samurai", 12), ("Sunglass", 15), ("Witchhat", 12)]),
  ("Petalsglobal", [("Daisy", 18), ("Fire", 13), ("Fishtank", 17), ("Lirium", 17), ("Sunflower", 20), ("Toxic", 15)])
  ];
  ...
  public func tokenIdentifier(c : Text, i : TokenIndex) : async TokenIdentifier {
	...
	func _mintNFT(request : MintRequest) : TokenIndex { //private mint for batch
  ...
  public shared(msg) func batchMintNFT(requests : [MintRequest]) : async [TokenIndex] {
    assert(msg.caller == _minter);
    var tokensMinted : [TokenIndex] = [];
    for(i in requests.vals()) {
      let tokenMinted : TokenIndex = _mintNFT(i);
      tokensMinted := Array.append(tokensMinted, [tokenMinted]);
    };
    return tokensMinted;
  };

  func getTrait(nums : [Nat]) : [Nat8] {  //DEPRECATED, DISCARDED OLD GENERATION MODEL
    var bgId : Nat8 = 0;
    var potId : Nat8 = 0;
    var stemId : Nat8 = 0;
    var petalId : Nat8 = 0;
...
/*     if (Option.isSome(_tokenListing.get(token))) {
  ...
  public query func getTokensByIds(vecs : [TokenIndex]) : async [(TokenIndex, Metadata)] {
    let tokens : [(TokenIndex, Metadata)] = Iter.toArray(_tokenMetadata.entries());
    var tokensbyId : [(TokenIndex, Metadata)] = [];
    for(i in vecs.vals()) {
      let thisdata : ?Metadata = _tokenMetadata.get(i);
      switch (thisdata) {
        case(?val) {
          tokensbyId := Array.append(tokensbyId, [(i, val)]);
        };
        case (_) {
  ...
  public query func http_request(request : HttpRequest) : async HttpResponse {
    ...
    switch(_getTokenData(_getParam(request.url, "type"))) {
      case (?metadata) {
        //let ids : [Nat8] = getTrait(metadata);
        //let padd = Blob.toArray(Text.encodeUtf8("<svg id='generated' style='width: 1280px;height: 1280px;' version='1.1' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' x='0px' y='0px' viewBox='0 0 1280 1280' xml:space='preserve'><image href='"));
        let padd = Blob.toArray(Text.encodeUtf8("<img src='"));
        let metastr = "https://storageapi.fleek.co/3b306168-5ec6-40d2-8042-2b46bff59592-bucket/" # Nat32.toText(metadata) #".png";
        let metaArr = Blob.toArray(Text.encodeUtf8(metastr));
        let metaBlob = Text.encodeUtf8(metastr);
        let suff = Blob.toArray(Text.encodeUtf8("' style='width:100vw;height:100%;max-width:100%;overflow:hidden;'></img>"));
      ...
      case (_) {
        switch(_getTokenData(_getParam(request.url, "tokenid"))) {
          case (?metadata) {
            ...
            let metaArr = Blob.toArray(Text.encodeUtf8(metastr));
            let metaBlob = Text.encodeUtf8(metastr);
            let suff = Blob.toArray(Text.encodeUtf8("'></img>"));
            let meta = Blob.fromArray(Array.append(Array.append(padd, metaArr), suff));
            return {
              status_code = 200;
              headers = [("content-type", "text/html")];
  ...
  func _getTokenData(tokenid : ?Text) : ?Nat32 {
    switch (tokenid) {
      case (?token) {
        ...
        let tokenind : TokenIndex = ExtCore.TokenIdentifier.getIndex(token);
